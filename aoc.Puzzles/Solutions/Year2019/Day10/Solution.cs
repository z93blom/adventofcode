...
namespace aoc.Puzzles.Solutions.Year2019.Day10 
{
    [Puzzle("Monitoring Station", 2019, 10)]
    class Solution : SolutionBase
    {
        ...
        public override async Task<object> PartTwo(string input, ChannelWriter<string> output)
        {
            var asteroids = GetAsteroids(input);
            CalculateSeenAsteroids(asteroids);
            var orderedBySeen = asteroids.OrderByDescending(kvp => kvp.Value.Count).ToArray();
